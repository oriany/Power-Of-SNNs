...
class LIFNeuron():
    ...
    def spike_generator(self, neuron_input):
        ...
        self.t = 0
        while self.t < duration-1:   
            if (Vm[self.t] > self.Vth):
                Vm[self.t+1 : self.t+self.tau_ref] = 0
                spikes[self.t] = 1
                self.t += self.tau_ref
            else:
                Vm[self.t+1] = Vm[self.t] + self.dt * ( -(Vm[self.t] - 0) +  neuron_input[self.t] * self.Rm) / self.tau_m
                spikes[self.t] = 0
        ...
        if self.debug:
            print ('LIFNeuron.spike_generator({}).exit_state(Vm={} at iteration i={}, time={})'
                   .format(self.neuron_label, self.Vm.shape, self.t, self.t))
